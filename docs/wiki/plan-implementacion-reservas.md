...
- [ ] **1.2 SQL**: crear tabla `restaurant_tables`:
  - `id BIGSERIAL PK`
...
- [ ] **1.3 SQL**: crear tabla `reservations`:
  - `id BIGSERIAL PK`
  - `restaurant_id BIGINT NOT NULL FK`
...
- [ ] **1.4 SQL**: crear tabla `reservation_notifications`:
  - `id BIGSERIAL PK`
...
- [ ] **1.11**: implementar `RestaurantTableJdbcDao` hasta que pasen los tests. `RowMapper` como `private static final ROW_MAPPER = ...`; `SimpleJdbcInsert` inicializado en constructor una sola vez. Joins/orden en SQL, no en Java.
- [ ] **1.12 TDD**: `ReservationJdbcDaoTest` — precondiciones vía `@Sql`; asserts con `JdbcTestUtils`/queries directas; **sin** `verify()`.
...
- [ ] **1.13**: implementar `ReservationJdbcDao` con misma disciplina (RowMapper estático, SimpleJdbcInsert en constructor). Ninguna query con `SELECT *`; columnas explícitas.
- [ ] **1.14**: correr `mvn clean test` completo; todo verde
- [ ] **1.15 Commit**: `feat(reservations): schema and DAOs for reservations and tables`
...
- [ ] **4.2**: implementar `ReservationServiceImpl.createReservation(CreateReservationData data, User authenticatedUser)` en una sola transacción. Algoritmo:
...
- [ ] **5.2**: implementar `cancelAsUser(reservationId)` con transición de estado. **No** chequea ownership (lo resuelve `AccessHelper.canCancelReservation` vía Security). Inserta fila en `reservation_notifications(type='CANCELLED')` dentro de la misma transacción; `@Async` dispara el mail leyendo esa fila.
- [ ] **5.3 TDD**: `AccessHelperTest.canCancelReservation`
...
- [ ] **5.3b TDD**: `AccessHelperTest.canViewUserReservations`
...
- [ ] **5.4 TDD**: `ReservationServiceImplTest.listUserReservations_paginated` (el servicio recibe el `userId` del target — quien figura en el slug — ya resuelto por el controller; no consulta `SecurityContextHolder`)
...
- [ ] **5.6 MVC test**: `UserReservationsControllerMvcTest`
...
- [ ] **5.7**: extender `WebAuthConfig` con reglas declarativas (SpEL contra `AccessHelper`, sin `@PreAuthorize`):
...
- [ ] **6.3 TDD**: `AccessHelperTest.canManageRestaurantReservations`
  - `returnsTrueWhenOwnerOwnsRestaurant`
  - `returnsTrueWhenUserIsAdmin`
  - `returnsFalseWhenOwnerDoesNotOwnRestaurant`
- [ ] **6.4**: implementar los 2 métodos en el servicio **sin** chequeos de ownership (la expression de Security los bloquea antes). Validaciones de estado sí (ej: no confirmar algo que ya está cancelado). Inserts en `reservation_notifications` antes de disparar mails `@Async`.
- [ ] **6.3b TDD**: `ReservationServiceImplTest.listReassignableTables(reservationId)` (feature v2-Q3)
...
- [ ] **6.3d TDD**: `ReservationServiceImplTest.listReservationsByRestaurant_respectsSort`
  - `sortRecent_returnsByDateDescSlotDesc` (más recientes primero; default)
  - `sortOldest_returnsByDateAscSlotAsc` (más antiguas primero)
...
- [ ] **6.3e TDD**: `ReservationServiceImplTest.findPageContainingReservation`
...
- [ ] **6.3c TDD**: `ReservationServiceImplTest.confirmAsOwnerWithReassignment`
...
- [ ] **6.5 MVC test**: `OwnerReservationControllerMvcTest`
...
- [ ] **6.7**: crear `owner/reservations.jsp`. Campos: usuario, fecha, slot, personas, mesa asignada (número + `type`), estado, comentario (`<c:out>`).
...
- [ ] **6.8**: proteger rutas en `WebAuthConfig`:
...
- [ ] **7.1 TDD**: `ReservationSchedulerImplTest` — **sin `verify()`**; todo lo assert es estado: count de filas por status y filas en `reservation_notifications`.
...
- [ ] **8.2**: agregar al contract `MailService`: `sendReservationConfirmed`, `sendReservationCancelled`, `sendReservationReminder`, `sendReservationCompleted`, `sendReservationPendingToOwner`. Implementar en `SmtpMailServiceImpl` usando el patrón existente. El destinatario de `sendReservationPendingToOwner` es el email del owner del restaurante (no el del reservante); el Locale es el del owner.
- [ ] **8.3 TDD**: tests de servicio verifican que cada transición **inserta la fila correcta** en `reservation_notifications`. **Prohibido `verify()`** sobre `MailService`. El stub de `MailService` en tests puede incrementar un contador interno y el assert es sobre ese contador o directamente sobre la tabla.
...
- [ ] **9.5**: auditar rutas de edición/borrado de reviews **antes** de tocar nada. Comandos concretos:
  - `grep -rn "@RequestMapping" webapp/src/main/java | grep -i review` — listar todos los endpoints de review
  - `grep -rn "reviewService\.update\|reviewService\.delete\|reviewService\.edit" webapp/src` — invocaciones desde controllers
  - `grep -rn "@PostMapping\|@DeleteMapping" webapp/src/main/java | grep -i review`
  Documentar los hallazgos en el cuerpo del PR de la Fase 9 (lista explícita: "endpoints encontrados: X/Y/Z; decisión: remover / restringir a ADMIN / dejar"). Si no hay endpoints de edición/borrado expuestos, dejarlo escrito igual ("no existen rutas públicas de edit/delete de reviews al 2026-04-16"). **Prohibido** asumir sin evidencia.
- [ ] **9.6**: modificar vista del restaurante (`restaurant.jsp`) para:
  - listar todas las reviews (legacy y nuevas) con `<c:out>` sobre comentarios
  - mostrar CTA "Dejar review" envuelto en `<sec:authorize access="@AccessHelper.canReviewRestaurant(authentication, #restaurantId)">`; el helper delega al servicio la consulta "¿tiene reserva completed sin review?". Nunca `<c:if test="${user.canReview}">` con lógica Java para decidir visibilidad de UI restringida por seguridad
- [ ] **9.7 MVC test**:
  - `post_review_withoutReservationId_returns400`
  - `post_review_withForeignReservation_returns403` (reserva existe pero no es tuya → 403, no 404)
  - `post_review_withCompletedOwnReservation_persistsReview`
  - `get_restaurantPage_asAnonymous_showsLegacyAndNewReviews` (sin CTA)
  - `get_restaurantPage_asUserWithCompletedReservation_showsCTA`
  - `get_restaurantPage_asUserWithoutCompletedReservation_hidesCTA`
...
- [ ] **10.3**: grep manual: ninguna vista muestra `${...}` sin `<c:out>` para valores dinámicos (comentarios de reserva, link externo, teléfono, nombre de restaurante). Ninguna URL hardcodeada sin `<c:url>`.
- [ ] **10.4**: logs con SLF4J en todos los servicios (creación, cancelación, confirmación, tick del scheduler). Patrón: `private static final Logger LOGGER = LoggerFactory.getLogger(ClaseActual.class);` y `LOGGER.info("reserva {} cancelada por user {}", id, userId)`. **Cero** `System.out`, `printStackTrace` ni concatenación con `+` dentro de logs.
- [ ] **10.5**: verificar que `WebAuthConfig` resuelve toda la autorización declarativa:
