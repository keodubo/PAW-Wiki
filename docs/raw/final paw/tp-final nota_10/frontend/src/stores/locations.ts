import { defineStore } from 'pinia';
import { ref, computed, reactive } from 'vue';
import type { Location } from '@/models';
import { ResourceCache } from '@/utils/ResourceCache';
import { LocationService } from '@/services';

export const useLocationsStore = defineStore('locations', () => {
  const locationService = new LocationService();

  const itemsMap = reactive(new Map<string, Location>());
  const listEtag = ref<string | null>(null);

  const cache = new ResourceCache<Location>(
    300,
    (uri, data) => itemsMap.set(uri, data),
    (uri) => itemsMap.get(uri),
  );

  const items = computed(() => Array.from(itemsMap.values()));

  function getEntry(uri: string): Location | undefined {
    return itemsMap.get(uri);
  }

  async function fetch(uri: string): Promise<Location> {
    return cache.get(uri);
  }

  async function fetchAll(): Promise<void> {
    const response = await locationService.list(listEtag.value);

    if (response.notModified) {
      return;
    }

    listEtag.value = response.etag || null;

    if (Array.isArray(response.data)) {
      for (const company of response.data) {
        if (company.selfUri) {
          cache.seed(company.selfUri, company, null);
        }
      }
    }
  }

  return {
    items,
    fetch,
    fetchAll,
    getEntry,
  };
});
