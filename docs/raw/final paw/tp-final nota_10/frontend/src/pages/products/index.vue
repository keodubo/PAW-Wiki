      ...
      <v-row>
            ...
            <v-card-text class="pa-4">
              <div class="mb-3">
                <h6 class="text-subtitle-2 font-weight-bold mb-1">
                  {{ $t('total') }}:
                  <span class="text-medium-emphasis">{{ totalProducts }} {{ $t('products') }}</span>
                </h6>
              </div>
              ...
              <div v-if="categories.length > 0" class="mb-3">
                ...
                <v-list class="pa-0" density="compact">
                  <v-list-item
                    ...
                    :key="category.id"
                    :class="{
                      'bg-primary-lighten-5': filters.categoryId === category.id,
                    }"
                  ...
                  >
                    ...
                    <v-list-item-title class="text-body-2"> {{ $t(`category.${category.name}`) }} ({{ category.productsCount }}) </v-list-item-title>
                  </v-list-item>
                </v-list>
              </div>
            </v-card-text>
        ...
        <v-col cols="12" md="9" lg="9" xl="9">
          ...
          <div v-if="products.length === 0 && !isLoading" class="text-center py-8">
            <v-img src="@/assets/empty.svg" alt="No products found" max-width="300" class="mx-auto mb-4" />
            <h3 class="text-h6 text-medium-emphasis mb-4">{{ $t('no_products_found') }}</h3>
            <v-btn v-if="hasActiveFilters" color="error" variant="outlined" @click="handleClearFilters" prepend-icon="mdi-filter-remove">
              {{ $t('clear_filters') }}
            </v-btn>
          </div>
          ...
          <v-row v-else>
            <template v-if="isLoading">
              <v-col v-for="n in 8" :key="`skeleton-${n}`" cols="12" sm="6" md="6" lg="4" xl="3">
                <v-skeleton-loader type="card" />
              </v-col>
            </template>

            <template v-else>
              <v-col v-for="data in products" :key="data.product.id" cols="12" sm="6" lg="6" xl="4">
                <ProductCard :product="data.product" :company="data.company" :category="data.category" />
              </v-col>
            </template>
          </v-row>
...
const totalProducts = computed(() => pagination.totalItems);
